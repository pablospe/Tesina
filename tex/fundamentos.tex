...
\begin{descripcion1}{Norma inducida:}
\begin{align}
\label{def:norma}
...
Se dice que $B_i$ y $B_j$ son ortogonales si se cumple que $\langle B_i, B_j \rangle = 0, \quad\quad \forall i\neq\,j$.
...
u_i & = \left( v_i - \sum_{j=1}^{i-1}{\langle v_i, u_j\rangle u_j} \right), \quad\quad i = 1,2,\dots,n
\end{align}
...
Polinomios ortogonales son clases de polinomios $\{B_{i}(t)\}$ definidos sobre el dominio $[a,b]$ que obedecen la relación de ortogonalidad
\begin{align}
...
\end{align}
los polinomios son \textit{\textbf{ortonormales}}. Es decir, están normalizados: $ \| B_i \| = 1$
...
Si tomamos como función peso $w(t)=1$ en la definición \refEQ{def:inner_product} de producto interno, es decir,
\begin{align*}
 \langle L_i, L_j \rangle & = \int_{-1}^{1}{L}_{i}(t) \,{L}_{j}(t)\,w(t)\,dt = \int_{-1}^{1}{L}_{i}(t) \,{L}_{j}(t)\,dt
\end{align*}
se pueden generar los polinomios de Legendre $\{L_i\}$ con el proceso de ortogonalización de Gram-Schmidt \refEQ{eq:Gram-Schmidt} en el intervalo $[-1,1]$. Normalizando de tal manera que $L_n(1)=1$ da los polinomios de Legendre esperados. Algunos de ellos son:
\begin{align*}
L_0(t) &= 1 \\[1ex]
L_1(t) &= t \\
...
Para nuestro propósito es conveniente utilizar los polinomios análogos definidos en $[0,1]$, los cuales son llamados \textit{\textbf{shifted} Legendre polynomials},
\begin{align}
\label{eq:legendre}
L_i(t) &= \sum_{j=0}^{i}{ C_{ij}\,t^j }
\end{align}
% donde los coeficientes $C_{ij}$ pueden calcularse con una fórmula cerrada, ver \cite{abramowitz70a}
el grado de $L_i$ es exactamente $i$.  Se conoce una fórmula cerrada para el cálculo de los coeficientes $C_{ij}$ ortonormales. A continuación se muestra dicha fórmula, ver \cite{abramowitz70a},
\begin{align}
...
% \noindent
Pueden verse los primeros diez polinomios de Legendre gráficamente en la figura~\ref{fig:legendre}.
\begin{figure}[!htbp]
\vspace*{-0.6cm}
\centering
...
Otros polinomios ortogonales que pueden usarse son los polinomios de Chebyshev. A diferencia de los de Legendre, la función peso es $w(t)=(1-x^2)^{-\frac{1}{2}}$. Se pueden generar estos polinomios con el proceso de ortogonalización de Gram-Schmidt. Se muestran en la figura~\ref{fig:chebyshev} algunos de ellos.
\begin{figure}[!htbp]
\vspace*{-0.9cm}
\centering
\includegraphics[scale=0.275]{imagen/chebyshev.pdf}
\vspace*{-0.6cm}
\caption{Primeros diez polinomios de Chebyshev, en el intervalo $[-1,1]$}
\label{fig:chebyshev}
\end{figure}
...
\begin{align*}
\label{ref:LS_inner_prod}
 \langle f, g \rangle_{\mbox{\footnotesize$LS$}} & = \int_{a}^{b}f(t)\,g(t)\,dt + \mu \, \int_{a}^{b}f'(t)\,g'(t)\,dt
\end{align*}
dando lugar a los polinomios de Legendre-Sobolev, ver \cite{LegendreSobolevComp}. En la figura \ref{fig:LS} pueden verse dos instancias con $\mu=\frac{1}{8}$ y $\mu=\frac{1}{16}$,
\begin{figure}[!htbp]
\vspace*{-0.9cm}
  \centering
  \subfloat[$\mu=\frac{1}{8}$]{\label{fig:LS_u_1_8}\includegraphics[scale=0.275]{imagen/legendre-sobolev_1_8.pdf}}
  \subfloat[$\mu=\frac{1}{16}$]{\label{fig:LS_u_1_16}\includegraphics[scale=0.275]{imagen/legendre-sobolev_1_16.pdf}}
  \caption{Primeros veinte polinomios de Legendre-Sobolev, en el intervalo $[0,1]$ }
  \label{fig:LS}
\end{figure}
...
%$L^2[a,b]$,
una función continua~$f$ en el dominio $[a,b]$ puede ser escrita como
