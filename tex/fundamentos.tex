...
En esta sección se introducirá algunos conceptos básicos que utilizaremos a lo largo del trabajo.
...
Si tomamos como función peso $w(t)=1$ en la definición (\ref{def:inner_product}) de producto interno, es decir,
\begin{align*}
 \langle L_i, L_j \rangle & = \int_{-1}^{1}{L}_{i}(t) \,{L}_{j}(t)\,w(t)\,dt = \int_{-1}^{1}{L}_{i}(t) \,{L}_{j}(t)\,dt
\end{align*}
se pueden generar los polinomios de Legendre $\{L_i\}$ con el proceso de ortogonalización de Gram-Schmidt en el intervalo $[-1,1]$. Normalizando de tal manera que $L_n(1)=1$ da los polinomios de Legendre esperados. Algunos de ellos:
\begin{align*}
L_0(t) &= 1 \\[1ex]
L_1(t) &= t \\
...
el grado de $L_i$ es exactamente $i$.  Se conoce una fórmula cerrada para el cálculos de los coeficientes $C_{ij}$ ortonormales. A continuación dicha fórmula, que puede encontrarse en \cite{abramowitz70a},
\begin{align}
...
\noindent
Si $\{B_i\}$ es una base de polinomios ortonormales respecto al producto interno (\ref{def:inner_product}),
%$L^2[a,b]$,
...
Los momentos matemáticos de una función $f$ definida en $[a,b]$ se definen como,
\begin{align}
\mu_{k} = \int_{a}^{b}{f(\lambda)\,\lambda^k\,d\lambda}
\end{align}
