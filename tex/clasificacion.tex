...
% 
% 
% In this case, additional slack variables ($\xi_k$) are introduced in the formulation of the problem in order to relax the constrains in (\ref{eq:svmpos}) and (\ref{eq:svmneg}) only when necessary. Then, the set of inequalities takes the following form
% \begin{equation} \label{eq:svm3}
% y_k[\omega^Tx_k+b]\geq1-\xi_k, \hspace{3mm} k=1,...,n .
% \end{equation}
% For an error to occur, the corresponding $\xi_k$ must exceed the unity, so $\sum_{k=1}^n\xi_k$ is an upper bound of the number of training errors. Then, changing the objective function to be minimized from $\|\omega\|_2$ to $\|\omega\|_2 + c\sum_{k=1}^n\xi_k$ will assign an extra cost for errors. The parameter $c$ is a penalization term where larger values of $c$ corresponds to higher penalty to errors.
% %The optimization problem, for a non-separable case becomes:
% %\begin{eqnarray} \label{eq:svm5}
% %\min_{\omega,b,\xi}  &  J_P(\omega,\xi)=\frac{1}{2}\omega^T\omega+c\sum_{k=1}^n\xi_k
% %\\
% %\mbox{s.t.}  &  y_k[\omega^Tx_k+b]  \geq  1-\xi_k, & k=1,...,n\nonumber
% %\\
% %  &  \xi_k  \geq  0, &  k=1,...,n.\nonumber
% %\end{eqnarray}
% 
% %We will now switch to a Lagrangian formulation of the problem There are two reasons
% %for doing this The rst is that the constraints .. will be replaced by constraints on the
...
% %which will allow us to generalize the procedure to the nonlinear case ..Section 
% 
% %Thus for an error to occur the corresponding i must exceed unity so Pi i is an upper
% %bound on the number of training errors Hence a natural way to assign an extra cost
% %for errors is to change the objective function to be minimized from kwk to kwk 
% %C ..Pi ik where C is a parameter to be chosen by the user a larger C corresponding to
% %assigning a higher penalty to errors
% 
